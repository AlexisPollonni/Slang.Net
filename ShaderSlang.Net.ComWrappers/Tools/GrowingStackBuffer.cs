...
internal unsafe ref struct GrowingStackBuffer(Span<byte> buffer) : IDisposable
{
    ...
    public GrowingStackBuffer(int capacity)
        : this(new Span<byte>(NativeMemory.Alloc((nuint)capacity), capacity))
    {
        _grewFromHeap = true;
    }
    ...
    public T* GetStructPtr<T>(in T value)
        where T : unmanaged
    {
        var span = Allocate(sizeof(T));
        if (!MemoryMarshal.TryWrite(span, in value))
            throw new InvalidOperationException("Failed to write struct to buffer");
        return (T*)PtrFromSpan(span);
    }

    public delegate TUnmanaged UnmanagedConverter<TManaged, out TUnmanaged>(
        in TManaged managed,
        ref GrowingStackBuffer buffer
    );

    public TUnmanaged* GetCollectionPtr<TManaged, TUnmanaged>(
        IEnumerable<TManaged>? collection,
        UnmanagedConverter<TManaged, TUnmanaged> converter,
        out uint count
    )
        where TUnmanaged : unmanaged
    {
        ...
        foreach (var item in collection)
        {
            ...
            if (!MemoryMarshal.TryWrite(span, converter(item, ref this)))
                throw new InvalidOperationException("Failed to write struct to buffer");
            count++;
        }
        ...
        where TUnmanaged : unmanaged
    {
        ...
        foreach (var item in collection)
        {
            ...
            if (!MemoryMarshal.TryWrite(span, unmanaged))
                throw new InvalidOperationException("Failed to write struct to buffer");
            count++;
        }
    ...
    public TUnmanaged* GetCollectionPtr<TUnmanaged>(
        IEnumerable<TUnmanaged>? collection,
        out uint count
    )
        where TUnmanaged : unmanaged
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static TUnmanaged UnmanagedToUnmanagedConverter(
            in TUnmanaged managed,
            ref GrowingStackBuffer growingStackBuffer
        ) => managed;
    }

    public TUnmanaged** GetComCollectionPtr<TManagedInterface, TUnmanaged>(
        IEnumerable<TManagedInterface>? comObject,
        out uint count
    )
