...
public readonly unsafe struct ShaderReflection
    : IEquatable<ShaderReflection>,
        INativeHandleMarshallable<ShaderReflection>
{
    ...
    internal ShaderReflection(Unmanaged.SlangProgramLayout* pointer)
    {
        ...
        Parameters = new NativeBoundedReadOnlyList<ShaderReflection, VariableLayoutReflection>
        {
            ...
            TryGetAt = (program, index) =>
                Reflection_GetParameterByIndex(program, checked((uint)index)),
        };
        TypeParameters = new NativeBoundedReadOnlyList<ShaderReflection, TypeParameterReflection>
        {
            ...
            TryGetAt = (program, index) =>
                Reflection_GetTypeParameterByIndex(program, checked((uint)index)),
        };
        EntryPoints = new NativeBoundedReadOnlyList<ShaderReflection, EntryPointReflection>
        {
            ...
            TryGetAt = (program, index) =>
                Reflection_getEntryPointByIndex(program, checked((nuint)index)),
        };
        HashedStrings = new NativeClassBoundedReadOnlyList<ShaderReflection, string>
        {
            ...
            TryGetAt = (program, index) =>
                Reflection_getHashedString(program, checked((nuint)index), out var count)?[
                    ..(int)count
                ],
        };
    }
    ...
    public ISession Session =>
        Reflection_GetSession(this)
        ?? throw new SlangException(
            "Could not retrieve Session from program layout, this should not happen!"
        );
    ...
    public TypeParameterReflection? FindTypeParameter(string name) =>
        Reflection_FindTypeParameter(this, name);
    ...
    public EntryPointReflection? FindEntryPointByName(string name) =>
        Reflection_findEntryPointByName(this, name);
    ...
    public ulong GlobalConstantBufferSize =>
        Reflection_getGlobalConstantBufferSize(this).ToUInt64();

    public TypeReflection? SpecializeType(
        TypeReflection type,
        IEnumerable<TypeReflection> specializationArgs,
    ...
    public TypeLayoutReflection? GlobalParamsTypeLayout =>
        Reflection_getGlobalParamsTypeLayout(this);
    public VariableLayoutReflection? GlobalParamsVarLayout =>
        Reflection_getGlobalParamsVarLayout(this);
}
