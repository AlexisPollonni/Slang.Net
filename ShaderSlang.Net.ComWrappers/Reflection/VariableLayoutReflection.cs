...
public readonly unsafe struct VariableLayoutReflection
    : IEquatable<VariableLayoutReflection>,
        INativeHandleMarshallable<VariableLayoutReflection>
{
    ...
    public override bool Equals(object? obj) =>
        obj is VariableLayoutReflection other && Equals(other);
    ...
    public VariableReflection Variable =>
        ...
        ?? throw new SlangException(
            $"{nameof(ReflectionVariableLayout_GetVariable)} returned a null pointer"
        );

    public string? Name => Variable.Name;

    public ModifierReflection? FindModifier(Unmanaged.ModifierID id) => Variable.FindModifier(id);

    public TypeLayoutReflection TypeLayout =>
        ReflectionVariableLayout_GetTypeLayout(this)
        ?? throw new SlangException(
            $"{nameof(ReflectionVariableLayout_GetTypeLayout)} returned a null pointer"
        );
    ...
    public VariableLayoutReflection? PendingDataLayout =>
        ReflectionVariableLayout_getPendingDataLayout(this);
}
