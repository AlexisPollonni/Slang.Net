...
public readonly record struct SessionDescription(
    ...
    : IMarshalsToNative<Unmanaged.SessionDesc>,
        IMarshalsFromNative<SessionDescription, Unmanaged.SessionDesc>,
        IFreeAfterMarshal<Unmanaged.SessionDesc>
{
    unsafe Unmanaged.SessionDesc IMarshalsToNative<Unmanaged.SessionDesc>.AsNative(
        ref GrowingStackBuffer buffer
    ) =>
        new()
        {
            structureSize = (nuint)sizeof(Unmanaged.SessionDesc),
            targets = buffer.GetCollectionPtr<TargetDescription, Unmanaged.TargetDesc>(
                Targets,
                out var targetCount
            ),
            ...
            >(PreprocessorMacros, out var preprocessorMacroCount),
            preprocessorMacroCount = (nint)preprocessorMacroCount,
            fileSystem = (Unmanaged.ISlangFileSystem*)
                ComInterfaceMarshaller<IFileSystem>.ConvertToUnmanaged(FileSystem),
    ...
    public static unsafe SessionDescription CreateFromNative(Unmanaged.SessionDesc unmanaged) =>
        new(
            InteropUtils.PtrToManagedMarshal<TargetDescription, Unmanaged.TargetDesc>(
                unmanaged.targets,
                (int)unmanaged.targetCount
            ),
            ...
            InteropUtils.PtrToManagedMarshal<
                PreprocessorMacroDescription,
                Unmanaged.PreprocessorMacroDesc
            >(unmanaged.preprocessorMacros, (int)unmanaged.preprocessorMacroCount),
