...
public class GeneratedOverloadsGenerator() : IncrementalGenerator(nameof(GeneratedOverloadsGenerator))
{
    private static void ProcessClass(SgfSourceProductionContext context, (CommonTypesContext, ITypeSymbol) pair)
    {
        ...
        foreach (var method in clazz.GetMembers().OfType<IMethodSymbol>())
        {
            ...
            if (originalMethodData.Signature.ReturnSig.Type.Equals(typeContext.SlangResultType,
                                                                   SymbolEqualityComparer.Default))
            {
            ...
            foreach (var data in extensionList.Where(data => !data.IsSignatureEquivalent(originalMethodData))
    ...
    {        
        context.RegisterPostInitializationOutput(initializationContext =>
        {
            ...
            initializationContext.AddSource($"{CommonTypesContext.GeneratedAttributesNamespace}.Attributes.g.cs",
                                            $$"""
...
{
    public static string GeneratedAttributesNamespace => typeof(GeneratedOverloadsGenerator).Namespace ?? throw new InvalidOperationException("Could not determine namespace for generated attributes");
    ...
    public CommonTypesContext(Compilation compilation)
    {
        ...
        IgnoreThrowingAttribute = FindSymbol(compilation, $"{GeneratedAttributesNamespace}.{IgnoreThrowingAttributeName}");
    }
