...
class GenerateCommands(ILogger<GenerateCommands> logger)
{
    /// <summary>
    /// Generate C# P/Invoke bindings and XML documentation for the Slang API.
    /// </summary>
    /// <param name="slangRepo">-r, Path to the local clone of the Slang repository.</param>
    /// <param name="bindingsOutput">-o, Directory where the generated C# bindings will be saved.</param>
    /// <param name="testsOutput">-t, Directory where the generated unit tests will be saved.</param>
    public void Bindings([Dir] DirectoryPath slangRepo, [Dir] DirectoryPath bindingsOutput, [Dir] DirectoryPath? testsOutput = null)
    {
        InstallDependencies();

        DirectoryExists(slangRepo).ShouldBeTrue($"Slang repository path '{slangRepo}' does not exist.");
        ...
        if (testsOutput is not null) CleanDirectory(testsOutput);
    ...
    /// <summary>
    /// Generate XML documentation for the Slang API.
    /// </summary>
    /// <param name="slangRepo">-r, Path to the local clone of the Slang repository.</param>
    /// <param name="xmlOutput">-o, Directory where the generated XML documentation files will be saved.</param>
    public void Docs([Dir] DirectoryPath slangRepo, [Dir] DirectoryPath xmlOutput)
    {
        InstallDependencies();
        ...
        CleanDirectory(xmlOutput);

        logger.LogInformation("Generating XML documentation files...");
        var genFiles = Generate([slangHeaderPath], xmlOutput, slangConfig, mode: PInvokeGeneratorOutputMode.Xml)
                            .Concat(Generate([slangGfxHeaderPath], xmlOutput, gfxConfig, mode: PInvokeGeneratorOutputMode.Xml))
                            .ToDictionary();

        WriteFilesToDisk(genFiles);
    }

    private static BuildConfig GetSlangConfig(DirectoryPath slangRepo) =>
        BuildConfig.GetBuildConfig("SlangApi", "slang") with
        {
            TraversalNames = [GetSlangPaths(slangRepo).SlangDeprHeaderPath, GetSlangPaths(slangRepo).SlangHeaderPath]
        };

    private static BuildConfig GetGfxConfig(DirectoryPath slangRepo) =>
        BuildConfig.GetBuildConfig("GfxApi", "gfx") with { TraversalNames = [GetSlangPaths(slangRepo).SlangGfxHeaderPath] };

    private void PrintClangVersion()
    {
        using var ver = clang.getClangVersion();
        using var ver2 = clangsharp.getVersion();

        logger.LogInformation("Generating CSharp bindings...");
        logger.LogInformation("CLANG VERSION: {Version}", ver);
        logger.LogInformation("CLANGSHARP VERSION: {Version}", ver2);
    }
    ...
    private static void InstallDependencies()
    {
        if (IsRunningOnLinux())
        {
            InstallTools("apt:?package=libc++-dev", "apt:?package=libc++abi-dev").ShouldNotBeNull().Length.ShouldBe(2);
        }
    }

    private static (FilePath SlangHeaderPath, FilePath SlangGfxHeaderPath, FilePath SlangDeprHeaderPath) GetSlangPaths(DirectoryPath slangRepo) =>
    (
    ...
    private Dictionary<FilePath, string> Generate(FilePath[] files,
                                                      ...
                                                      PInvokeGeneratorOutputMode mode = PInvokeGeneratorOutputMode.CSharp)
    {
...

        {
            ...
            {
                var translationUnit
                    = CreateTranslationUnitForFile(filePath, generator.IndexHandle, clangCmdArgs.ToArray(), translationFlags);

                if (translationUnit is null)
                {
                    logger.LogWarning("Skipping \'{FilePath}\' due to one or more errors listed above", filePath);
                    error = true;
                    continue;
                }
                ...
                try
                {
                    ...
                    generator.GenerateBindings(translationUnit, MakeAbsolute(filePath).ToString(), [.. clangCmdArgs],
                                               translationFlags);
                }
                catch (Exception e)
                {
                    logger.LogError(e, "Failed to generate bindings for \'{FilePath}\'", filePath);
                    error = true;
                }
            }
    ...
    private void WriteFilesToDisk(IDictionary<FilePath, string> outputFiles)
    {
        foreach (var (path, textBody) in outputFiles)
        {
            logger.LogDebug("Writing file \'{FilePath}\' to disk", path);
    ...
    private TranslationUnit? CreateTranslationUnitForFile(FilePath filePath,
                                                          CXIndex index,
                                                          ReadOnlySpan<string> clangCmdArgs,
                                                          CXTranslationUnit_Flags flags)
    {
        var translationUnitError = CXTranslationUnit.TryParse(index, MakeAbsolute(filePath).ToString(),
                                                              clangCmdArgs, [], flags,
                                                              out var handle);

        if (translationUnitError != CXErrorCode.CXError_Success)
        {
            logger.LogError("Parsing failed for '{FilePath}' due to '{TranslationUnitError}'", filePath,
        ...
        {
            logger.LogInformation("Diagnostics for '{FilePath}':", filePath);
            ...
            for (uint i = 0; i < handle.NumDiagnostics; ++i)
            {
                ...
                logger.Log(level, "#{Index} : {Diagnostic}", i, diagStr);
...
static class ClangAstExtensions
{
    public static StrDic GetAdditionalRemappedNames(TranslationUnit translationUnit)
    {
        ...
        var externCursors = FindAllChildrenOfKind<LinkageSpecDecl>(rootCursor).ShouldNotBeNull().ToArray();
    ...
    public static IEnumerable<(string, string)> GetEnumAdditionalRemappedNames(IEnumerable<LinkageSpecDecl> linkageSpecDecls)
    {
        ...
        var remappedEnumNames = foundEnums
            .Select(decl =>
            {
                ...
                var formatedName = decl.Spelling.StartsWith("Slang") ? decl.Spelling[5..] : decl.Spelling;
        ...
        var remappedEnumMembers = foundEnums
                                  ...
                                  .Select(decl =>
                                  {
                                      ...
                                      var parentEnumNameWords = parentEnumName.Humanize().Transform(To.LowerCase).Split(' ');

                                      var declWords = decl.Spelling.Humanize().Transform(To.LowerCase).Split(' ');

                                      var humanizeDeclWords = declWords.Index()
    ...
    public static IEnumerable<(string, string)> GetDisambiguatedRemappedNames(TranslationUnitDecl translationUnit)
    {
    ...
    public static Dictionary<string, Guid> GetComStyleClassUuids(TranslationUnit translationUnit)
    {
        ...
        var guids = guidMethods.Select(m => (m.Parent.ShouldNotBeNull().Name, GetGuidFromTypeGuidMethod(m))).ToDictionary();
    ...
    public static T? FindFirstChildOfKind<T>(this Cursor cursor) where T : Cursor
    {
        return FindAllChildrenOfKind<T>(cursor).FirstOrDefault();
    }

    public static IEnumerable<T> FindAllChildrenOfKind<T>(this Cursor cursor) where T : Cursor
    {
    ...
    public static string GetFullQualifiedNameFromCursor(this Cursor cursor)
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(currentCursor.Spelling)) nameParts.Push(currentCursor.Spelling);
    ...
    FilePathCollection? TraversalNames = null)
{
    ...
    public PInvokeGeneratorConfiguration ToGeneratorConfiguration(DirectoryPath outputDir,
                                                                  DirectoryPath? testsOutputDir = null,
                                                                      ...
                                                                      = PInvokeGeneratorOutputMode.CSharp)
    {
        ...
        if (IsRunningOnUnix()) opts |= GenerateUnixTypes;

        return new(Language, "c++17", DefaultNamespace, MakeAbsolute(outputDir).ToString(), null, mode, opts)
        {
            DefaultClass = DefaultClass,
            LibraryPath = LibraryPath,
            MethodPrefixToStrip = PrefixStrip,
            TraversalNames = [.. TraversalNames.Select(t => t.MakeAbsolute(Context.Environment).ToString())],
    ...
    static string GetSlangPlatformDefine()
    {
        ...
        {
            if (IsRunningOnWindows()) return "SLANG_WIN64";
            if (IsRunningOnLinux()) return "SLANG_LINUX";
            if (IsRunningOnMacOs()) return "SLANG_OSX";
        }
        else
        {
            if (IsRunningOnWindows()) return "SLANG_WIN32";
        }

        throw new PlatformNotSupportedException($"Unsupported platform for Slang bindings generation {Context.Environment.Platform}");
    }
